...
class CNNNetwork(nn.Module):
    def __init__(self):
        ...
        self.layer2 = nn.Sequential (
            nn.Conv1d(32, 32, kernel_size=5, padding=1, dilation=1),
            nn.BatchNorm1d(32),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
    ...
    def forward(self, x):
        ...
        avg_pool = torch.mean(x6, dim=2)
        max_pool = torch.max(x6, dim=2).values
        std_pool = torch.std(x6,dim=2)

        avg_pool_early = torch.mean(x2, dim=2)
        max_pool_early = torch.max(x2, dim=2).values
        std_pool_early = torch.std(x2,dim=2)
        curve_features   = torch.cat([avg_pool, max_pool,std_pool,avg_pool_early,max_pool_early,std_pool_early], dim=1)#  3*32+3*128
...
class CNNModel:
    def __init__(self,
                 model_path='../models/convolution_dilated_version_experimental.pth',
    ...
    def predict(self, df: pd.DataFrame, model_path=None,threshold=0.5) -> pd.DataFrame:
    ...
    def retrain_with_k_fold(self, train_df: pd.DataFrame, k=10, epochs=100, patience=15):
        ...
        # Spara till CSV-fil
        csv_save_path = '../models/cnn_dilated_experimental_kfold_metrics.csv'
        ...
        print(generate_latex_table(df_metrics))
