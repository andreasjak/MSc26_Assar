...
class CNNNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        std_pool        = torch.std(curve, dim=2)
        curve_features  = torch.cat([avg_pool, max_pool,std_pool], dim=1)  # (n, 3*128)
...
class CNNModel:
    def __init__(self,
                 model_path='../models/cnn_global_max_avg_std_pooling.pth',
                 scaler_path='../models/scaler.pkl'):
        self.model_path = model_path
        self.model = CNNNetwork().to(device)
        self.scaler = joblib.load(scaler_path)
        total_params = sum(p.numel() for p in self.model.parameters())
        self.name = 'cnn_global_maxpooling'
    ...
    def retrain_with_k_fold(self, train_df: pd.DataFrame, k=10, epochs=100, patience=15):
        ...
        # Spara till CSV-fil
        csv_save_path = '../models/cnn_global_max_avg_std_pooling_kfold_metrics.csv'
