...
class CNNModel:
    def __init__(self,
                 model_path='../models/cnn_dataset_experiment.pth',
    ...
    def evaluate(self,df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['prediction'] = (df['cnn_probability'] > 0.5).astype(int) 
    ...
    def retrain_with_k_fold(self, train_df: pd.DataFrame, k=10, epochs=100, patience=15,scaling_indices= None,filename='dataset_experiment'):
        """
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X_dummy, y_dummy)):
            ...
            X_train = build_X(fold_train_df)
            X_val   = build_X(fold_val_df)
            ...
            X_val[:,scaling_indices] = scaler.transform(X_val[:,scaling_indices])

            y_train = torch.tensor(np.array(fold_train_df['label'].values, dtype=np.int64))
            y_val   = torch.tensor(np.array(fold_val_df['label'].values,   dtype=np.int64))

            X_train, X_val = map(torch.tensor, [X_train, X_val])

            fold_train_dl = DataLoader(TensorDataset(X_train, y_train, torch.tensor(fold_train_df['row_id'].to_numpy())), batch_size=512, shuffle=True)
            fold_val_dl   = DataLoader(TensorDataset(X_val,   y_val,   torch.tensor(fold_val_df['row_id'].to_numpy())),   batch_size=512)
            
            
            # Definiera unikt filnamn för den bästa modellen i denna fold
            fold_model_path = f"../models/{filename}_fold{fold+1}.pth"
        ...
        # Spara till CSV-fil
        csv_save_path = f'../models/{filename}_kfold_metrics.csv'
    ...
    def retrain_with_k_fold_no_scaling(self, train_df: pd.DataFrame, k=10, epochs=100, patience=15,scaling_indices= None,filename='dataset_experiment'):
        """
        Tränar modellen med Stratified K-Fold Cross-Validation.
        Sparar train_loss, val_loss, val_acc och val_auc för varje fold i en Pandas DataFrame och CSV.
        """
    ...
    def retrain(self, train_dl, val_dl, epochs=10000, patience=5,model_path=None,weights=[1.0,3.0]):
